...
| Fork CI: upstream sync, ozone image build on the fork, changeset check skipped, image builds triggered from `eurosky`          | `d6dc37ef4` + this PR | `sync-upstream.yaml`, `build-and-push-ozone-ghcr.yaml`, `repo.yaml`, `build-and-push-eurosky-*.yaml` | Ours only                    |
