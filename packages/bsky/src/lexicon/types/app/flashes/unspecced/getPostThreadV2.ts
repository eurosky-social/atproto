...
import type * as AppFlashesUnspeccedDefs from './defs.js'

const is$typed = _is$typed,
  validate = _validate
const id = 'app.flashes.unspecced.getPostThreadV2'
...
export interface ThreadItem {
  $type?: 'app.flashes.unspecced.getPostThreadV2#threadItem'
  uri: string
  /** The nesting level of this item in the thread. Depth 0 means the anchor item. Items above have negative depths, items below have positive depths. */
  depth: number
  value:
    | $Typed<AppFlashesUnspeccedDefs.ThreadItemPost>
    | $Typed<AppFlashesUnspeccedDefs.ThreadItemNoUnauthenticated>
    | $Typed<AppFlashesUnspeccedDefs.ThreadItemNotFound>
    | $Typed<AppFlashesUnspeccedDefs.ThreadItemBlocked>
    | { $type: string }
}
